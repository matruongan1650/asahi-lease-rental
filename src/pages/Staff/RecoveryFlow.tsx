...
export interface RecoveryFlowProps {
  o: {
    ...
    phone: string;
    contact: string;
    products: Array<{ id: string; qr: string; name: string; expected: number; icon: string; image?: string }>;
    note?: string;
    rawOrder?: any;
  };
...
export default function RecoveryFlow({ o, onComplete, onExit }: RecoveryFlowProps) {
  ...
  const [prods, setProds] = useState<any[]>(
    o.products.map(p => ({ ...p, scanned: false, counted: p.expected, report: [] }))
  );
              ...
              <Card key={p.id} pad={13} style={{ borderColor: p.scanned ? (hasIssue ? "var(--danger-bright)" : "var(--border)") : "var(--border)", opacity: p.scanned ? 1 : 0.5 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
                  ...
                  {p.scanned
                    ...
                    : <span style={{ fontSize: 12, color: "var(--fg-subtle)", fontWeight: 700 }}>未スキャン</span>}
                </div>
                {p.scanned && (
  ...
  if (step === 4) {
    ...
    footer = <Btn full size="lg" icon="warehouse" onClick={() => onComplete(o.firestoreId || o.id, signed, prods.flatMap(p => p.report?.flatMap(r => r.photos || []) || []))}>倉庫へ戻る</Btn>;
  }

  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: "var(--bg)", minHeight: 0 }}>
      <TopBar title={step === 4 ? "完了" : o.site} sub={o.id} onBack={step === 4 ? undefined : onExit} right={step < 4 ? <IconBtn name="phone" onClick={() => window.open(`tel:${o.phone || "090-0000-0000"}`)} /> : null} />
